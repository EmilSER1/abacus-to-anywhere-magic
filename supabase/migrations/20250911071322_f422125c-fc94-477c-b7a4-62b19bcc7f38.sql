...
WITH room_names AS (
  SELECT DISTINCT 
    ...
    TRIM(REGEXP_REPLACE("Помещение/Кабинет", '\s+', ' ', 'g')) as room_name
  FROM turar_medical 
  WHERE "Отделение/Блок" IS NOT NULL AND "Помещение/Кабинет" IS NOT NULL
  UNION
  SELECT DISTINCT 
    TRIM(REGEXP_REPLACE("ОТДЕЛЕНИЕ", '\s+', ' ', 'g')) as dept_name,
    TRIM(REGEXP_REPLACE("НАИМЕНОВАНИЕ ПОМЕЩЕНИЯ", '\s+', ' ', 'g')) as room_name
  FROM projector_floors 
...
SELECT d.id, rn.room_name
FROM room_names rn
JOIN public.departments d ON d.name = rn.dept_name
WHERE rn.room_name != '';
