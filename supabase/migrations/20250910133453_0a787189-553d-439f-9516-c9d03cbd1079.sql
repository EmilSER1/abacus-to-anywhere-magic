...
CREATE TABLE IF NOT EXISTS public.mapped_projector_rooms (
  ...
  -- Поля для связок с Турар (пока без FK)
  linked_turar_room_id UUID,
...
CREATE TABLE IF NOT EXISTS public.mapped_turar_rooms (
  ...
  -- Поля для связок с проектировщиками (пока без FK)
  linked_projector_room_id UUID,
  is_linked BOOLEAN NOT NULL DEFAULT false
);
...
ALTER TABLE public.mapped_turar_rooms 
ADD CONSTRAINT fk_linked_projector_room 
FOREIGN KEY (linked_projector_room_id) REFERENCES mapped_projector_rooms(id) ON DELETE SET NULL;
