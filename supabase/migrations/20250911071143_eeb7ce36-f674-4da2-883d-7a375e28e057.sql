...
SELECT DISTINCT TRIM(REGEXP_REPLACE(dept_name, '\s+', ' ', 'g'))
FROM (
  SELECT "Отделение/Блок" as dept_name FROM turar_medical WHERE "Отделение/Блок" IS NOT NULL
  UNION
  SELECT "ОТДЕЛЕНИЕ" as dept_name FROM projector_floors WHERE "ОТДЕЛЕНИЕ" IS NOT NULL AND TRIM("ОТДЕЛЕНИЕ") != ''
) combined_departments
WHERE TRIM(REGEXP_REPLACE(dept_name, '\s+', ' ', 'g')) != ''
ORDER BY TRIM(REGEXP_REPLACE(dept_name, '\s+', ' ', 'g'));

-- Заполняем справочную таблицу кабинетов
INSERT INTO public.rooms (department_id, name)
SELECT DISTINCT d.id, TRIM(REGEXP_REPLACE(room_name, '\s+', ' ', 'g'))
FROM (
  SELECT DISTINCT 
    TRIM(REGEXP_REPLACE("Отделение/Блок", '\s+', ' ', 'g')) as dept_name,
    "Помещение/Кабинет" as room_name 
  FROM turar_medical 
  WHERE "Отделение/Блок" IS NOT NULL AND "Помещение/Кабинет" IS NOT NULL
  UNION
  SELECT DISTINCT 
    TRIM(REGEXP_REPLACE("ОТДЕЛЕНИЕ", '\s+', ' ', 'g')) as dept_name,
    "НАИМЕНОВАНИЕ ПОМЕЩЕНИЯ" as room_name 
  FROM projector_floors 
  WHERE "ОТДЕЛЕНИЕ" IS NOT NULL AND "НАИМЕНОВАНИЕ ПОМЕЩЕНИЯ" IS NOT NULL 
    AND TRIM("ОТДЕЛЕНИЕ") != '' AND TRIM("НАИМЕНОВАНИЕ ПОМЕЩЕНИЯ") != ''
) combined_rooms
JOIN public.departments d ON d.name = combined_rooms.dept_name
WHERE TRIM(REGEXP_REPLACE(room_name, '\s+', ' ', 'g')) != ''
ORDER BY d.name, TRIM(REGEXP_REPLACE(room_name, '\s+', ' ', 'g'));
...
WHERE d.name = TRIM(REGEXP_REPLACE(tm."Отделение/Блок", '\s+', ' ', 'g'))
  AND r.name = tm."Помещение/Кабинет";
...
WHERE d.name = TRIM(REGEXP_REPLACE(pf."ОТДЕЛЕНИЕ", '\s+', ' ', 'g'))
  AND r.name = pf."НАИМЕНОВАНИЕ ПОМЕЩЕНИЯ";
