...
-- 1. Create a view for user management WITHOUT email addresses
...
-- 2. Policy for regular admins to view user management data (NO EMAIL ACCESS)
CREATE POLICY "Verified admins can view user management" 
ON public.user_management_view
FOR SELECT 
USING (public.is_verified_admin());

-- 3. Create audit table for role changes with proper security
...
-- 4. Create secure role update function with auditing
...
-- 5. Create a trigger to prevent direct role updates outside of the secure function
CREATE OR REPLACE FUNCTION public.prevent_role_update()
RETURNS TRIGGER AS $$
BEGIN
  -- Allow the system/super admin function to update roles
  IF current_setting('role', true) = 'supabase_admin' OR 
     current_setting('application_name', true) = 'secure_role_update' THEN
    RETURN NEW;
  END IF;
